...
# This is Samhith's High Low Guess game Alexa Skill.
...
def launch_request_handler(handler_input):
    ...
    speech_text = (
        "Welcome to the High Low guessing game brought to you by RR Games and Company! Say yes to continue")
...
def number_guess_handler(handler_input):
    ...
    session_attr['status']='waiting'
    
    session_attr["no_of_guesses"] = session_attr["no_of_guesses"]+1 #Increment number of guess by 1

    if guess_num > target_num:
        speech_text = (
            "{} is too high. Try saying a smaller number. Do you want to continue? Say yes or no".format(guess_num))
        reprompt = "Try saying a smaller number."
        handler_input.response_builder.speak(speech_text).ask(reprompt)

    elif guess_num < target_num:
        speech_text = (
            "{} is too low. Try saying a larger number. Do you want to continue? Say yes or no".format(guess_num))
...
def number_guess_handler(handler_input):
    ...
    speech_text = (
         "Your have tried to guess the random number {} times. Now, try again! Please say a number between 1 and 100.".format(attempts))
...
def no_handler(handler_input):
    ...
    a new game.
    """
    speech_text = "Thanks. Bye Bye! The RR Games & Company hopes you have a great day!!"

    handler_input.response_builder.speak(speech_text).set_should_end_session(True)
    return handler_input.response_builder.response
    

# Ignore everything below here for now. Not important in Samhith's High Low Skill----------------------------------------------------------------
