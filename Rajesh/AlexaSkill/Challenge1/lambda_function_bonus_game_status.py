...
def launch_request_handler(handler_input):
    ...
    session_attr['guess_number'] = 50#random.randint(0, 100)#Generate the random number and store in the session
    session_attr['no_of_guesses'] = 0#Put the value in the session
    session_attr['status'] = 'playing'#put the value in the session
...
def number_guess_handler(handler_input):
    ...
    # type: (HandlerInput) -> Response
    session_attr = handler_input.attributes_manager.session_attributes
    target_num = session_attr["guess_number"]#read guess_number value from session
    guess_num = int(handler_input.request_envelope.request.intent.slots[
        "number"].value)
    
    #If game status is not playing the exit the function
    game_status=session_attr['status']
    if game_status!='playing':
        handler_input.response_builder.speak( "Say yes to continue or no to stop.").ask( "Say yes to continue or no to stop.")
        return handler_input.response_builder.response
        
    #Before starting the game lets change the game status to waiting
    session_attr['status']='waiting'#put the value in the session
    
    session_attr["no_of_guesses"] = session_attr["no_of_guesses"]+1 #Increment number of guess by 1 in the session
    if guess_num > target_num:
        speech_text = (
            "{} is too high. Try saying a smaller number.Say yes to continue or no to stop.".format(guess_num))
        reprompt = "Try saying a smaller number."
        handler_input.response_builder.speak(speech_text).ask(reprompt)

    elif guess_num < target_num:
        speech_text = (
            "{} is too low. Try saying a larger number.Say yes to continue or no to stop.".format(guess_num))
...
#This is NumberGuessIntent
@sb.request_handler(can_handle_func=lambda input:is_intent_name("TellMyStatus")(input))
def number_guess_handler(handler_input):
    """Handler for processing TellMyStatus Intent."""
    # type: (HandlerInput) -> Response
    session_attr = handler_input.attributes_manager.session_attributes
    attempts = session_attr["no_of_guesses"]
    speech_text = (
         "Here is your count so far {}. Please say a number between 1 and 100.".format(attempts))
